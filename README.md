...
Then open `http://localhost:8080/OBOS_Home_Plus_V17.html` in Chrome.
...
> The application uses the `anthropic-dangerous-direct-browser-access: true` header to allow direct browser-to-API calls. This is appropriate for demo and pilot use. For production deployment, replace the client-side API calls with a thin server-side proxy.
